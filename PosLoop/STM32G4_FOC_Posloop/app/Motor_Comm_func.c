...
void Motor_Load_UART_Tx_Buff(void)
{
    ...
    temp_f[3] = (float)rtU.PosRef;
    temp_f[4] = (float)PosFd;
    temp_f[5] = (float)rtU.Encoder_Theta;
    temp_f[6] = (float)0;
...
void Motor_UART_Rx_Data_Decoding(void)
{
	...
	if(m_app.UART_RxBuff.DataDecodingState == 1)
	{
		if(split_string((char *)m_app.UART_RxBuff.UartRxBuffer,'=',&Frame_header,&Frame_data) == 0x00)
		{
			...
			if(m_app.UART_RxBuff.UartRxBuffer[0] == 'P' && m_app.UART_RxBuff.UartRxBuffer[1] == 0x33)
			{
				m_app.UART_Cmd.PosRef = (int16_t)strtof;
			}
